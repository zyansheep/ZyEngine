...
Object::Object(const std::shared_ptr<VertexArray>& vertexArray, Shader& shader, std::string modelUniform)
: m_VertexArray(vertexArray), m_Shader(shader){
	m_MatrixLocation = shader.GetUniformLocation(modelUniform.c_str());
}
Object::Object(const std::shared_ptr<VertexArray>& vertexArray, Shader& shader, std::vector<Texture*> textures, std::string modelUniform)
: m_VertexArray(vertexArray), m_Shader(shader), m_Textures(textures){
	m_MatrixLocation = shader.GetUniformLocation(modelUniform.c_str());
}

void Object::Render(const Camera& viewport){
	...
	if(MatrixArray.size() > 1){
		...
		const std::unique_ptr<Buffer>& buffer = m_VertexArray->InstanceBuffer;
		if(buffer == NULL){
			m_VertexArray->InstanceBuffer.reset( new Buffer({{ShaderType::Mat4, "a_transform"}}) );
